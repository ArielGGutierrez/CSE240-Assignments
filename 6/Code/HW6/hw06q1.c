...
int add(char* bookTitle_input, char* author_input, char* booktype_input, unsigned int bookId_input, unsigned int aisle_input)
{
	...
	/* Create a new library record node */
	//------------------------------------------------------------------------------------------//
	/* Check if there is available memory to create the node */
	struct libraryRecord* newBook = (struct libraryRecord*)malloc(sizeof(struct libraryRecord));
	if (newBook == 0)
	{
		return 2;
	}

	/* Start adding stuff to the node */
	strcpy(newBook->bookTitle, bookTitle_input);
	strcpy(newBook->author, author_input);
	...
	/* Finish adding stuff to new node */
	newBook->booktype = type;
	newBook->bookId = bookId_input;
	newBook->aisle = aisle_input;
	newBook->next = NULL;
	//------------------------------------------------------------------------------------------//

	/* Try to add new node to the list */
	//------------------------------------------------------------------------------------------//
	/* Check if the maximum amount of books has been reached */
	if (count < MAX_BOOKS)
	{
		/* Check if list is empty */
		if (tempList != NULL)
		{
			/* Check if book is in library record already */
			for (int i = 0; i < count; i++)
			{
				/* Book is in the library record, so don't add it */
				if ((strcmp(tempList->bookTitle, bookTitle_input) == 0) || (tempList->bookId == bookId_input))
				{
					free(newBook);
					return 0;
				}

				/* If there is another book in the list after this one, scroll to it */
				if (tempList->next != NULL)
				{
					tempList = tempList->next;
				}
			}
			tempList->next = newBook; // Add new book into the end
		}

		/* List is empty, make the head start at the new book */
		else
		{
			list = newBook;
		}
	...
	/* Max number of books has been reached, don't add the new book */
	else
	{
		free(newBook);
		return 2;
...
void sort()
{
	struct libraryRecord* iterator = list;		// work on a copy of 'list'
	
	/* Don't need to sort a list that is empty or has 1 thing */
	if (list != NULL && count != 1)
	{
		/* Temporary variables to store values */
		char tempTitle[MAX_NAME_LENGTH];
		char tempAuthor[MAX_NAME_LENGTH];
		bookType tempBooktype;
		unsigned int tempBookId;
		unsigned int tempAisle;

		for (int i = 0; i < count; i++) // Element to be checked
		{
			while(iterator->next != NULL) // Makes sure process doesn't go out of bounds
			{
				if (iterator->bookId > iterator->next->bookId) // If this element is bigger than the next, swap them
				{
					/* Copy stuff from current node to the temp variable */
					strcpy(tempTitle, iterator->bookTitle);
					strcpy(tempAuthor, iterator->author);
					tempBooktype = iterator->booktype;
					tempBookId = iterator->bookId;
					tempAisle = iterator->aisle;

					/* Move stuff from next node to this node */
					strcpy(iterator->bookTitle, iterator->next->bookTitle);
					strcpy(iterator->author, iterator->next->author);
					iterator->booktype = iterator->next->booktype;
					iterator->bookId = iterator->next->bookId;
					iterator->aisle = iterator->next->aisle;

					/* Move stuff from temp variable to the next node */
					strcpy(iterator->next->bookTitle, tempTitle);
					strcpy(iterator->next->author, tempAuthor);
					iterator->next->booktype = tempBooktype;
					iterator->next->bookId = tempBookId;
					iterator->next->aisle = tempAisle;
				}

				iterator = iterator->next; // Scroll to the next spot
			}
			iterator = list; // Return iterator to the beginning of the list
		}
	}
...
int delete(unsigned int bookId_input)
{
	...
	/* No need to do anything if there isn't anything on the list */
	if (list != NULL)
	{
		/* Iterate through the list */
		while(tempListCur != NULL)
		{
			/* Check if the bookId matches */
			if (tempListCur->bookId == bookId_input)
			{
				/* If the number to delete is in the beginning, move the head */
				if (tempListCur == list)
				{
				...
				/* If not, move the previous pointer's next pointer to after the current pointer */
				else
				{
			...
			tempListCur = tempListCur->next; // Move the current iterator up
		}
	}
...
void load(char* fileName)
{
	...
	if (file != NULL)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			/* Check if there is enough memory to put the new node */
			if (newBook == 0)
			{
				return;
			}

			/* Fill the new node with stuff that is read */
			fread(newBook->bookTitle, sizeof(newBook->bookTitle), 1, file);
			fread(newBook->author, sizeof(newBook->author), 1, file);
			fread(&newBook->booktype, sizeof(newBook->booktype), 1, file);
			fread(&newBook->bookId, sizeof(newBook->bookId), 1, file);
			fread(&newBook->aisle, sizeof(newBook->aisle), 1, file);
			newBook->next = NULL;
			...
			/* Place the new node at the end of the list and move the pointer to the new end of the list */
			else
			{
		...
		printf("Books record loaded from Book_list.txt.\n"); // Let the user know that information was loaded
	}

	/* The file doesn't exist, no need to load anything */
	else
	{
